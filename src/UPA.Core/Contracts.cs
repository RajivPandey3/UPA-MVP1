...
public readonly record struct EntityId(string Value)
{
    public static EntityId FromStableKey(string key)
    {
        ...
        return new EntityId(Convert.ToHexString(bytes)[..32].ToLowerInvariant());
    }
...
public sealed record ScanContext(string ProjectRoot, bool ReadOnly = true);
    ...
    IReadOnlyList<Diagnostic> Diagnostics)
{
    ...
    public IReadOnlyList<AssemblyDefinitionInfo> Assemblies { get; init; } = Array.Empty<AssemblyDefinitionInfo>();
    public string ProjectName { get; init; } = string.Empty;
    public string ProjectRoot { get; init; } = string.Empty;
    public IReadOnlyList<string> ProjectSettingsFiles { get; init; } = Array.Empty<string>();
    public IReadOnlyList<string> BuildTargetHints { get; init; } = Array.Empty<string>();
}

public sealed record PackageInfo(string Name, string? Version);
public sealed record AssemblyDefinitionInfo(string Path, string Name);
