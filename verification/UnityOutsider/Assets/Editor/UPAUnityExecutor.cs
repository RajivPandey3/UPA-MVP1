...
namespace UPA.UnityExecutor.Editor
{
    ...
    {
        private static readonly Type[] SupportedComponents =
        {
            typeof(Rigidbody), typeof(BoxCollider), typeof(SphereCollider), typeof(CapsuleCollider),
            typeof(MeshCollider), typeof(Camera), typeof(Light), typeof(AudioSource),
        ...
        public static Type ResolveComponentType(string name)
        {
            foreach (var component in SupportedComponents)
            {
                if (string.Equals(name, component.Name, StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(name, component.FullName, StringComparison.Ordinal) ||
                    string.Equals(name, component.AssemblyQualifiedName, StringComparison.Ordinal))
                    return component;
            }
            return null;
        }
            ...
            UpaUnityExecutionResult result)
        {
            ...
            if (mutation.Kind == UpaUnityMutationKind.AddComponent &&
                ResolveComponentType(mutation.ComponentTypeName) == null)
            {
                result.Errors.Add(
                    "UNITY-EXEC-013: Component type is not in the supported component catalog.");
            }
            ...
            UpaUnityExecutionResult result)
        {
            ...
            var type = ResolveComponentType(mutation.ComponentTypeName);
            if (type == null ||
